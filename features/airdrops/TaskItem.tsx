...
export const TaskItem: React.FC<TaskItemProps> = ({
  ...
  return (
    <div className={`
      ...
      ${task.completed ? 'bg-gray-100 dark:bg-gray-800 opacity-70' : 'bg-white dark:bg-card-dark hover:shadow-md'}
      ${isCompletionDisabled ? 'opacity-60 cursor-not-allowed' : ''}
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-start flex-grow min-w-0">
            {task.subTasks && task.subTasks.length > 0 && (
              <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="mr-1 p-1 self-center" aria-label={isExpanded ? `Collapse sub-tasks for ${task.description}` : `Expand sub-tasks for ${task.description}`}>
            ...
            <div 
                className={`flex-shrink-0 mt-0.5 ${isCompletionDisabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
          ...
          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-0.5 sm:gap-1 flex-shrink-0">
            ...
            {!isArchived && (
              <>
                ...
                <Button variant="ghost" size="sm" onClick={() => onOpenTimerModal(task)} title={t('taskitem_timer_aria', { taskName: task.description, defaultValue: `Log time for ${task.description}`})} aria-label={t('taskitem_timer_aria', { taskName: task.description, defaultValue: `Log time for ${task.description}`})} className="p-1"><Clock size={14}/></Button>
                <Button variant="ghost" size="sm" onClick={() => onOpenTaskForm(task)} title={t('taskitem_edit_aria', { taskName: task.description, defaultValue: `Edit task ${task.description}`})} aria-label={t('taskitem_edit_aria', { taskName: task.description, defaultValue: `Edit task ${task.description}`})} className="p-1"><Edit3 size={14} /></Button>
                <Button variant="ghost" size="sm" onClick={() => onOpenTaskForm(undefined, task.id)} title={t('taskitem_add_subtask_aria', { parentTaskName: task.description, defaultValue: `Add sub-task to ${task.description}`})} aria-label={t('taskitem_add_subtask_aria', { parentTaskName: task.description, defaultValue: `Add sub-task to ${task.description}`})} className="p-1"><PlusCircle size={14} /></Button>
              </>
            )}
             <Button variant="ghost" size="sm" onClick={handleDelete} title={t('taskitem_delete_aria', { taskName: task.description, defaultValue: `Delete task ${task.description}`})} aria-label={t('taskitem_delete_aria', { taskName: task.description, defaultValue: `Delete task ${task.description}`})} className="text-red-500 hover:text-red-700 dark:hover:text-red-400 p-1"><Trash2 size={14} /></Button>
          </div>
        </div>
